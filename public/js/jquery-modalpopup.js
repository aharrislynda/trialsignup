    ...
    open: function() {
        ...
        try {
            ...
            jQuery("#modal-overlay, #modal-close-link").click(function() {
                ...
                setTimeout("modalWindow.close()", 0);
            });
            ...
            if(typeof (lynda.isIMobileDevice)!='undefined')
            {
...
var openTouchScrollableJQModalExtended = function(url, width, height, title, type, data, iframe, contentSelector) {
	if(lynda.isIMobileDevice)
	{
		openJQModalExtended(url, width, height, '', '', null, false);
		setTouchScroll(contentSelector);
	}
	else
		openJQModalExtended(url, width, height, '', '', null, iframe);
}

function setTouchScroll(contentSelector) {
	if (jQuery('#modal-contents').html() != '') {
		jQuery(contentSelector).addClass('scrollableContent');
		jQuery('#modal-contents').addClass('scrollableContainer');
		jQuery(contentSelector).touchScroll();
	}
	else
		setTimeout(function() {setTouchScroll(contentSelector)}, 10);
}