...
validateCharacters = function (value, regex) {
...
removeInvalidCharacters = function (obj, regex) {
    var validString = validateCharacters(obj.value, regex);
    if (obj.value !== validString) {
        obj.value = validString;
    }
}
var tags = new function () {
	...
	this.populate = function (itemtypeid, itemid, parentElement, callback) {
		...
		jQuery.ajax({ url: lynda.baseUrl + "ajax/tags.aspx", type: 'GET', data: parameters, dataType: 'json', cache: false, success: function (data) {
			var d = simpleEval(data);
			var listItems = '';
			if (d && d.tags) {
...
var bookmark = new function() {
					...
					if (data && data.data) {
						for (tid in data.data) {
							...
							if (_type)
								...
								if (_type[pid + bookmarkIdentifierIconState]) _type[pid + bookmarkIdentifierIconState](data.data[tid][pid], pid);
						}
					}
				};
		...
		jQuery('.' + css).each(function(i) {
			...
			if (d.ty == 3) {
				if (!iconStates[d.ty][d.i + '_' + d.p + bookmarkIdentifierIconState]) {
					iconStates[d.ty][d.i + '_' + d.p + bookmarkIdentifierIconState] = function(id, courseId) {
						if (e.html() == '') {
							e = getElementByCourseId(courseId, css);
						}
						self.initLink(e, false, { bookmark: id });
					};
				...
				if (!iconStates[d.ty][d.i + bookmarkIdentifierIconState]) {
					iconStates[d.ty][d.i + bookmarkIdentifierIconState] = function(id, courseId) {
						if (e.html() == '') {
							e = getElementByCourseId(courseId, css);
						}
						self.initLink(e, false, { bookmark: id });
			...
			//hide all chapters and videos if SHOWBOOKMARKONLY. UnHide later if they are bookmarked or their children are bookmarked
			if (showBookmarkedOnly == "1") {
				if (d.ty == 2) //videos
	...
	var getElementByCourseId = function(courseId, css) {
		var e;
		jQuery('.' + css).each(function(i) {
			if (courseId == simpleEval(jQuery(this).attr('json')).i) {
				e = jQuery(this);
			}
		});
		return e;
	...
	this.initLink = function(element, create, opt) {
		...
		{
			if (!create) {
				if (d.ty == 2)   //movie (show video and show its corresponding chapter name)
				{
					e.parent().parent().removeClass('hideDetail');  //show video row
					jQuery('#table_' + e.parent().parent().attr('name')).attr('style', 'display:block'); //show table containing the video
				...
				else if (d.ty == 3) //timecode
				{
					...
					if (!(jQuery('#' + e.parent().parent().parent().attr('name')).find('div.bkm3').hasClass('active')))
						jQuery('#' + e.parent().parent().parent().attr('name')).find('div.bkm3').attr('style', 'display:none;'); //hide bkmk icon if video not bookmarked
					jQuery('#' + e.parent().parent().parent().attr('name')).removeClass('hideDetail'); //show video
				}
			}
		}
		...
		if(d.th) initParameters.isCourseDetailsBookmark=1;
		if (initParameters.isIPad == 1) {
			...
			if(d.th)
				if(create)
					jQuery('#'+d.th).unbind('click').unbind('mouseover').unbind('mouseout').attr('json',jQuery.toJSON(d)).attr('title','Create bookmark').click(function() { return self.quickSaveIPad(this,{ type: d.ty,id: d.i,position: d.p,title: d.t,breadcrumbs: d.b,bi: d.bi }); });
				else
					jQuery('#'+d.th).unbind('click').unbind('mouseover').unbind('mouseout').attr('json',jQuery.toJSON(d)).attr('title','Delete bookmark').click(function() { return self.quickDeleteIPad(this,{ type: d.ty,id: d.i,position: d.p,title: d.t,breadcrumbs: d.b,bi: d.bi }); });
			...
			jQuery('#bookmark').hover(function() {
				...
				if (!isModalDisplayed) {
					...
					jQuery('#bookmark-description-max').html('300 characters left');
	...
	this.details = function() {
		...
		var isBookmarkBelowScreen = function() {
			var docViewTop = jQuery(window).scrollTop();
			var docViewBottom = docViewTop + jQuery(window).height();

			var elemTop = offset.top;
			var elemBottom = elemTop - 4 + Number(cssStates[5].height.replace('px', ''));
		...
		else if (isBookmarkBelowScreen()) {
			state(5);
			modal.css({ 'top': (offset.top - modal.height() + 22) + 'px', 'z-index': '9999' });
			buttons.css({ 'position': 'relative', 'top': (386 + 20) + 'px', 'margin-top': '-20px' });
		}
		else {
			if (self.currentState != 0) {
				if (self.currentState == 5) {
					modal.css({ 'top': (offset.top + modal.height() - 22) + 'px' });
					buttons.css({ 'position': 'relative', 'top': '0px', 'margin-top': '0px' });
				}
				state(0);
			}
			else {
	...
	this.remove = function() {
		...
		var elem=jQuery(e).prev();
		var h=elem.attr('json');
		if(!h||h.length==0)
			h=elem.html();

		if(d.th&&h&&h!='')
			self.initLink(jQuery(e).prev(),true,{ bookmark: 0 });
		else
	...
	this.deleteShow = function() {
		if(initParameters.isIPad||initParameters.isplayer){
			self.remove();
			return false;
		}
		var e = baseElement;
...
function IsNameValid(name) {
	var pattern = new RegExp("^[A-Za-z\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u1FFF]([-'A-Za-z\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u1FFF\\s\\.]*[A-Za-z\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u1FFF\\s\\.]|[A-Za-z\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u1FFF]*)$");
	return pattern.test(name);
}
function reset_multi_user_form()
{
	if(jQuery('input#txtFirstName').length!=0)
	{
		jQuery('input#txtFirstName').val('');
	}
	if(jQuery('input#txtLastName').length!=0)
	{
		jQuery('input#txtLastName').val('');
	}
	if(jQuery('input#txtCompany').length!=0)
	{
		jQuery('input#txtCompany').val('');
	}
	if(jQuery('input#txtTitle').length!=0)
	{
		jQuery('input#txtTitle').val('');
	}
	if(jQuery('input#txtPhone').length!=0)
	{
		jQuery('input#txtPhone').val('');
	}
	if(jQuery('textarea#txtDescription').length!=0)
	{
		jQuery('textarea#txtDescription').val('');
	}
	if(jQuery('input#txtEmail').length!=0)
	{
		jQuery('input#txtEmail').val('');
	}
	if(jQuery('input#txtFirstName').length!=0)
	{
		jQuery("div#divInput").addClass("overlay_content").show();
		jQuery("div#divSuccess").removeClass("overlay_content").hide();
	}
}
/************************************************************
...
function overlay()
{
	...
	this.show=function(e,width,height,title,center)
	{
		if(jQuery("#modal_bg").length>0)
		{
			...
			jQuery('.overlay_holder').hide();
			jQuery(".error_type").each(function()
			{
				jQuery(this).removeAttr("style");
			});
			jQuery(".overlay_master_content").addClass("overlay_content").show();
			jQuery(".success_message").removeClass("overlay_content").hide();
		}
		var myValues=jQuery('body').width();
		var myWidth=(Number(myValues)-Number(width));
		var myNum=myWidth/2;

		if(center==true)
		{
			var myValuesHeight=jQuery(window).height();
			var myHeight=(Number(myValuesHeight)-Number(height));
			var myNumHeight=myHeight/2;
		}

		jQuery('#modal_bg').attr('display','none');
		jQuery("<div/>",{
			id: "modal_bg",
			css: {
		...
		jQuery('#modal_bg').fadeTo('fast','0.50');
		if(navigator.platform=='iPad'||navigator.platform=='iPhone'||navigator.platform=='iPod')
		{
			jQuery("#modal_bg").css("top",jQuery(window).scrollTop());
		};
		if(jQuery("#overlay_bar").length>0)
		{
			jQuery("#overlay_bar").remove();
		}
		if(title!=""||title!=undefined||title!=null)
		{
			jQuery("<div/>",{
				id: "overlay_bar",
				html: '<h5>'+title+'</h5><a href="javascript:void(0);" class="overlay_close"><img src="/ui/images/enterprise_redesign/ent_re_icon_close.gif" alt=""/></a>'
			}).prependTo("#"+e);
		} else
		{
			jQuery("<div/>",{
				id: "overlay_bar",
				html: '<a href="javascript:void(0);" class="overlay_close"><img src="/ui/images/enterprise_redesign/ent_re_icon_close.gif" alt=""/></a>'
			}).prependTo("#"+e);
		}
		jQuery(".overlay_close").click(function()
		{
			jQuery(".main_overlay").fadeOut();
			jQuery("#"+e).fadeOut();
		...
		jQuery('div#errorsOccured').hide();
		jQuery("#"+e).hide();
		jQuery("#"+e).fadeTo("slow",1);
		jQuery("#"+e).show();
		jQuery("#"+e).css('left',myNum);
		jQuery("#"+e).css('width',width);
		if(center==true)
		{
			jQuery("#"+e).css('top',myNumHeight+jQuery(window).scrollTop());
		} else
		{
			jQuery("#"+e).css('top',25+jQuery(window).scrollTop());
		}
		jQuery("#"+e).addClass('main_overlay');
	...
	var set_defaults = function (overlay_defaults) {
		...
		var watcher = function() {
			jQuery(master_overlay_class).each(function() {
				jQuery(this).click(function() {
					master_overlay_element = jQuery(this).attr('lang');
					show_certain(jQuery(this).attr('lang'), jQuery(this).attr('title'));
				});
			});
			jQuery(".overlay_close").click(function() {
				jQuery(".main_overlay").fadeOut();
		...
		var reposition = function () {
			jQuery("#modal_bg").css('width', jQuery(window).width());
			jQuery("#modal_bg").css('height', jQuery(window).height());
			var myValues = jQuery('body').width();
			...
			if (master_overlay_how == 'fade') {
				jQuery("#" + master_overlay_element).css('left', myNum);
				if (master_center == true) {
					jQuery("#" + master_overlay_element).css('top', myNumHeight + jQuery(window).scrollTop());
				} else {
					jQuery("#" + master_overlay_element).css('top', master_top + jQuery(window).scrollTop());
				}
			} else if (master_overlay_how == 'tween') {
				if (master_center == true) {
					var repo_top = (myNumHeight + jQuery(window).scrollTop());
				} else {
					var repo_top = (master_top + jQuery(window).scrollTop());
				}
				jQuery("#" + master_overlay_element).animate({
					left: myNum,
					top: repo_top
				}, 150);
			}
		...
		var show_certain = function (e, title) {
			...
			jQuery('#modal_bg').fadeTo(master_fade_speed, master_fade_amount);
			if (navigator.platform == 'iPad' || navigator.platform == 'iPhone' || navigator.platform == 'iPod') {
				jQuery("#modal_bg").css("top", jQuery(window).scrollTop());
			};
			if (jQuery("#overlay_bar").length > 0) {
			...
			if (master_overlay_how == 'fade') {
				...
				jQuery("#" + e).show();
				jQuery("#" + e).css('left', myNum);
				jQuery("#" + e).css('width', master_overlay_width);
				if (master_center == true) {
					jQuery("#" + e).css('top', (myNumHeight + jQuery(window).scrollTop()));
				} else {
					jQuery("#" + e).css('top', master_top + jQuery(window).scrollTop());
				}
			} else if (master_overlay_how == 'tween') {
				jQuery("#" + e).css('opacity', '1');
				jQuery("#" + e).css('width', '1px');
				jQuery("#" + e).css('height', '1px');
				jQuery("#" + e).css('overflow', 'hidden');
				jQuery("#" + e).hide();
				jQuery("#" + e).css('left', myNum);
				if (master_center == true) {
					jQuery("#" + e).css('top', myNumHeight);
				} else {
					jQuery("#" + e).css('top', master_top + jQuery(window).scrollTop());
				}
		...
		var close = function () {
			...
			if (master_overlay_how == 'fade') {
				...
				jQuery("#" + master_overlay_element).fadeOut();
			} else if (master_overlay_how == 'tween') {
...
var IEDropdown = function() {
	this.myElement = null;
	this.myDomElement = null;

	this.initialize = function(element, width) {
		if (jQuery.browser.msie && (parseInt(jQuery.browser.version, 10) >= 6) && (parseInt(jQuery.browser.version, 10) < 9)) {
			this.myElement = jQuery('#' + element);
			if(this.myElement.length == 0) return;
			this.myElement.parent().css({ "overflow": "hidden", "width": width });
			this.myDomElement = this.myElement.get(0);
			this.originalWidth = this.myDomElement.style.width;
			this.dontWiden = false;
			this.myElement.mousedown(this.widen);
			this.myElement.blur(this.shrink);
			this.myElement.change(this.shrink);
		}
	};

	this.widen = function(e) {
		if (this.dontWiden) return;
		var styledWidth = IEDropDownFixSingleton.myDomElement.offsetWidth;
		IEDropDownFixSingleton.myDomElement.style.width = 'auto';
		var desiredWidth = IEDropDownFixSingleton.myDomElement.offsetWidth;
		...
		if (desiredWidth < styledWidth) {
			IEDropDownFixSingleton.dontWiden = true;
			IEDropDownFixSingleton.myDomElement.style.width = this.originalWidth;
			IEDropDownFixSingleton.myDomElement.click(); // Simulate another click, since setting styles has already caused the box to close at this point.
		}
	};

	this.shrink = function(e) {
		IEDropDownFixSingleton.myDomElement.style.width = IEDropDownFixSingleton.originalWidth;
	};

};
var IEDropDownFixSingleton = new IEDropdown();
